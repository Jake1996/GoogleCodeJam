			...
			for(int index=0;index<n;index++) {
				...
				for(int i=0;i<256;i++) {
					...
					if(index==0) {
						...
						minCost = Math.min(minCost, dp[index-1][0]+Math.abs(numbers[index]-i)); //delete all and get required no					
						minCost = Math.min(minCost, dp[index-1][0]+deleteCost+insertCost); //delete everything and insert
						if(min==0)
						minCost = Math.min(minCost,Math.abs(numbers[index]-i)+dp[index-1][index(i)] );
						else
						for(int j =0;j<256;j++) {
							if(min!=0)
								minCost =  Math.min(minCost, dp[index-1][index(j)]+insertCalc(i,j)*insertCost+Math.abs(numbers[index]-i)); //modify and insert multiple						
						}
					}
